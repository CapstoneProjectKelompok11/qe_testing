...
public class DeleteBuilding {
    ...
    @Step("I set an endpoint for DELETE building")
    public String setDeleteEndpointBuilding(){
        return base_url + "/admin/building?buildingId={buildingId}";
    }

    @Step("I request DELETE detail building")
    public void requestDeleteDetailBuilding(String status, int buildingId) throws IOException {
        ...
        if (status.equals("not authorized")) {
            SerenityRest.given()
                    ...
                    .when().delete(setDeleteEndpointBuilding());
        } else {
            SerenityRest.given()
                    .header("Content-Type", "application/json")
    ...
    @Step("I validate the status code for delete building is {int}")
    public void validateStatusCode(int statusCode){
        SerenityRest.then().statusCode(equalTo(statusCode));
    }

    @Step("validate the data detail after delete building")
    public void validateDataDetailAfterDeleteBuilding(String message){
