...
public class GetProfileImage {
    ...
    @Step("I set an endpoint for GET profile image")
    public String setGetEndpointProfileImage() {
        return base_url + "/profile/image/{filename}";
    }

    @Step("I request GET detail profile image")
    public void requestGetDetailProfileImage(String filename) throws IOException {
        if (filename.equals("valid")) {
            this.filename = FileUtils.readFileToString(new File(System.getProperty("user.dir") + "//src//test//resources//filejson//profileImage.json"), StandardCharsets.UTF_8);
        ...
        SerenityRest.given()
                ...
                .when().get(setGetEndpointProfileImage());
    }

    @Step("I validate the status code is for get profile image is {int}")
    public void validateTheStatusCode(int statusCode) {
        SerenityRest.then().statusCode(equalTo(statusCode));
    }

    @Step("validate the data details of profile image")
    public void validateTheDataDetailsOfProfileImage(String message) {
